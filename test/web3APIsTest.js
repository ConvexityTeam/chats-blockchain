...
async function TestingWeb3APIs() {
  try {
    ...
    const AddAdmin = await trnx.addAdmin(AdminAcc.address).then((result) => {
      return result;
    });
    ...
    // Add the above created account as an Admin
    const AddAuthorizer = await trnx.addAuthorizer(AuthoriserAcc.address).then((result) => {
      return result;
    });
    console.log("8) Authoriser Added >>>", AddAuthorizer);

    const UserList = await trnx.isUserListed(User.address).then(async (result) => {
      return result;
    });
    console.log("9) User is Listed >>> ", UserList);

    const AdminList = await trnx.isAdmin(AdminAcc.address).then(async (result) => {
      return result;
    });
    console.log("10) Admin is Listed>>> ", AdminList);

    const AuthList = await trnx.isAuthorizer(AuthoriserAcc.address).then(async (result) => {
      return result;
    });
    console.log("11) Auth is Listed >>> ", AuthList);

    const BlackList = await trnx.isBlackListed(User.address).then(async (result) => {
      return result;
    });
    ...
    const GetAdminList = await trnx.getAdminList(AdminAcc.address).then(async (result) => {
      return result;
    });
    ...
    const Approved = await trnx.approve(User.address, User.privateKey, AuthoriserAcc.address, '10000000000000000000').then(async (result) => {
      return result;
    });
    console.log("16) Approved  >>> ", Approved);

    const Allowance = await trnx.allowance(User.address, AuthoriserAcc.address).then(async (result) => {
      return result;
    });
    ...
    const TransferAdmin = await trnx.transferAdmin(User.address, '10000000000000000000' ).then((result) => {
      return result;
    });
    ...
    const Minted = await trnx.minting('1000000000000000000', User.address).then((result) => {
      return result;
    });
    ...
    const DestroyBlackFund = await trnx.destroyBlackFunds(User.address).then((result) => {
      return result;
    });
    ...
    const RemoveAuthorizer = await trnx.removeAuthorizer(AuthoriserAcc.address).then((result) => {
      return result;
    });
    console.log("29) Evil Authoriser removed from List >>>", RemoveAuthorizer);

    const RemoveAdmin = await trnx.removeAdmin(AdminAcc.address).then((result) => {
      return result;
    });
